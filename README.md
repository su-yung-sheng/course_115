...
`11502/level.html` 的內容寬度是 **`max-w-5xl`**,步驟列用 **`flex-wrap`**(不是橫向捲動)。

> ⚠️ 步驟從五個長到七個之後,舊的 `max-w-3xl` ＋ `overflow-x:auto` 會把最後一個切掉一半。
> **捲動比切掉更糟**:沒有捲軸提示,學生不會知道右邊還有東西 ——
> 他看到的只是一個壞掉的畫面。換行永遠不會切,最多排成兩列。

只加寬步驟列沒有用 —— 那樣步驟會比底下的內容寬,看起來像跑版,
所以連內容一起加寬(投影出來也比較好讀)。手機(<520px)再縮小字和內距。
