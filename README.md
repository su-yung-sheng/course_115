...
| **概念檢測**(程式拼圖前的五題) | `shared/quiz.js` | **一個單元 6 小時一次** | 整份退回老師的題庫,學生**感覺不到** |

概念檢測不是「一個學生問一次」——那樣一個班就能燒完一天的預算
(3 題 × 30 人 × 10 關 = 900 次)。做法是**一次要一整個題池**(6 題),
快取 6 小時,每個學生從池子裡隨機抽 3 題、選項順序也不一樣。

> ⚠️ AI 出的一定是**四選一選擇題＋正解索引**,判分在學生的瀏覽器裡。
> 改成開放式問答就變成「AI 決定成績」,而 AI 會失守、會過載、會額度用完。

---

## 🧠 概念檢測與星數上限

每一關在「程式拼圖」**之前**要先考五題:老師題庫 2 題 ＋ AI 出的 3 題。
...
程式做出來了但概念沒懂 → 2 星。規則寫在 `shared/grading.js` 的
...
在 `11502/content/blocks.js` 該關卡裡加 `quiz: [...]`,**至少 5 題**:

```js
quiz: [
  { q: '題目？',
    options: ['正解', '誤解一', '誤解二', '誤解三'],
    answer: 0,                       // 正解在 options 裡的索引
    why: '答錯的人還沒懂什麼' }      // 這句會顯示在「還沒穩的地方」
]
```

- **少於 5 題的關卡不辦概念檢測**(直接跳過這一步)。AI 出事時要退得回題庫,
  湊題目會出現重複題
- 選項會在畫面上洗牌,不必擔心正解都排在同一個位置
- `why` 要講**概念**,不要只講「你錯了」—— 說不出哪裡不懂的話,他重讀也只是再看一遍
- 改完題目記得在 Apps Script 編輯器執行一次 **`clearCache`**,
  否則 AI 還會照舊重點出題(題池快取 6 小時)
