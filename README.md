...
`shared/answer.js` 會比對**連續 12 個字一模一樣**(`COPY_RUN`,
和 `warmup.js` 的抄題判定同一個數字)。
⚠️ 門檻要夠寬:學生用到題目裡的詞是正常的
(「副程式」「正方形」本來就只有那個講法),所以看的是連續字串,
不是「有沒有用到那些字」。
...
- ⚠️ **沒有圈選題的那幾問,要先寫一句(至少 6 字)才給「下一題」**。
  只放一顆按鈕的話,學生一路按下去,五問完全沒看 —— 這一步就沒有意義了。
  但這裡**不判對錯**,寫了就能往下:這一段是「想一想」,不是關卡
  (真正的門檻在概念檢測)。判對錯的話學生會開始猜系統要什麼字,
  那正好毀掉這一步。寫完只給正向回饋(「✓ 你講到了…」),
  **不給**「你還沒講到 X」—— 後者會直接把答案講出去
...
1. **照訂單組套餐**(10 張抽 3,一張一張來)→ 每一張都是同樣三個位置
2. **換掉一個模組**(只換飲料,動到別格會被擋下並說明)→ 改一格,其他不動
3. **打開主餐,換掉裡面一個零件** → 麵包 ＋ 主料 ＋ 蔬菜 ＋ 醬料

第三關對上的是「副程式裡面可以再呼叫副程式」——
主餐自己也是四個插槽拼出來的(`PARTS`:麵包／主料／蔬菜／醬料,共 14 個選項),
而且**不同主餐共用同一種零件**,那正是「重複的部分可以共用」最直接的樣子。

> ★ 第三關要**真的換掉一個零件**,不是打開來看看就好 ——
> 看圖說故事學不到東西。換哪一格都可以。
