...
/* ===================================================================
   規則本體的指紋
   -------------------------------------------------------------------
   ⚠️ 老師 2026-08-26 問「為什麼檔案版本是 8-25？」——
      因為那次只補了說明文字，規則本體仍然是更早的版本。
      **日期分不出「改註解」和「改規則」**，這正是問題所在。
   ⇒ 檔頭放一行指紋，只算剝掉註解之後的規則本體。
     老師拿 Firebase Console 上的那一行跟這裡比對，
     就知道線上是不是同一份；而且寫再多註解都不會讓它變動。
   =================================================================== */
section('★★ 檔頭的指紋要和規則本體對得上');
{
  const crypto = require('crypto');
  /* ⚠️ 剝註解的方式要和產生那行指紋時一致：
     先剝 block 註解，再剝整行的 //，最後剝行尾的 //，去掉空行。 */
  const body = SRC
    .replace(/\/\*[\s\S]*?\*\//g, '')
    ...
    .join('\n');
  const want = crypto.createHash('sha1').update(body, 'utf8')
                     .digest('hex').slice(0, 8);
  const m = SRC.match(/規則本體指紋：([0-9a-f]{8})/);
  ok(!!m, '★ 檔頭有「規則本體指紋：xxxxxxxx」這一行');
  ok(!!m && m[1] === want,
     '★★ 指紋對得上（檔頭 ' + (m ? m[1] : '無') + '／實際 ' + want + '）'
     + (m && m[1] !== want ? ' —— 改過規則就要更新檔頭那一行' : ''));
  const n = SRC.match(/規則本體指紋：[0-9a-f]{8}　（(\d+) 行/);
  ok(!!n && Number(n[1]) === body.split('\n').length,
     '★ 行數也要對得上（檔頭 ' + (n ? n[1] : '無') + '／實際 '
     + body.split('\n').length + '）');
}
...
/* ===================================================================
   檔頭的「最後更新」那一行
   -------------------------------------------------------------------
   ⚠️⚠️ 這裡本來還有一條「檔頭日期不可落後於內文的日期標記」。
      2026-08-26 移除，因為它**證明是脆弱的**：
      只要在註解裡寫一句「2026-08-25 出現過這種落差」，
      內文就多出一個比檔頭新的日期，測試立刻紅 ——
      而規則本體根本沒動。誤報久了就會有人把檢查關掉。
   ⇒ 改用上面那組**指紋**：它只算剝掉註解的規則本體，
     寫再多說明都不會變，而且分得出「改註解」和「改規則」。
     這一節只留「那行還在」，因為它是人類讀得懂的版本標示。
   =================================================================== */
section('★ 檔頭要有人看得懂的版本標示');
{
  ok(/規則本體最後更新：\d{4}-\d{2}-\d{2}/.test(SRC),
     '★ 檔頭有「規則本體最後更新：YYYY-MM-DD」');
}
