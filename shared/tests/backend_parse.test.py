...
# ⚠️⚠️ 老師 2026-09-07 問：「只有文字為什麼會如此（慢）？」
#    ★ 送出去的不是「一點文字」：每次都完整送一遍評分規則、
#      空白範本虛擬碼、參考解答虛擬碼、約兩千字防禦規則、學生虛擬碼。
#    ⚠️ 我同一天已經在 OCR 記憶體那邊猜錯一次 ——
#      這裡先印數字，不要憑印象斷言「prompt 太大」。
#      動 prompt 會直接影響分數，不是可以隨便試的地方。
ok('[批改] 送出' in _core_src,
   '★★★ 每次批改要印出實際字數（規則／範本／參考解答／學生各多少）')
...
ok('_n500[0] >= 2' in _aa2,
   '★★★ 同一個模型連續兩次 500 也要換模型 —— 2026-09-09 實測連續四次 500'
   '（含兩次掛住 48／46 秒）而備援一次都沒被試過')
ok('_n500[0] = 0' in _aa2 and 'else:' in _aa2,
   '★★ 只算**連續**的 500：中間夾到別種錯誤要重新計數，'
   '否則整場累積下來會在不相干的時候誤觸')
...
ok('attempt >= max_attempts - 2' in _aa_ret,
   '★★ 連續 500 時最後一次改用純文字 —— Gemma 對結構化輸出本來就比較不穩')
ok('所有 API Key 額度已耗盡' not in _core_src,
   '★★ 放棄時要講**最後一次的真正原因**，不要一律說成「額度耗盡」—— '
   '那會把 500 誤導成額度問題')

# ⛔ 「和空白範本完全一樣 → 0 分」是字串相等比對，後端自己做就好。
#    prompt 一次送三份完整虛擬碼，其中一份是白送的 ——
#    而 500 INTERNAL 正是請求太大時最典型的症狀。
ok('_tpl_for_ai' in _core_src,
   '★★★ 空白範本不要整份送給模型（省下的通常是三分之一的輸入）')
ok('_stu_s == _tpl_s' in _core_src,
   '★★★ 相等比對要在後端做 —— 規則沒有改變，只是換個地方判斷，而且更準')
_blank = _core_src[_core_src.index('_stu_s = '):][:1600]
ok('"score": 0' in _blank,
   '★★ 完全相同時直接回 0 分，連 API 都不必打（學生也不用等）')
ok('省下 %d 字' in _core_src,
   '★★ 要把省下多少字印出來 —— 不然沒有人知道這一刀有沒有效')

# ⚠️⚠️ 2026-09-08 老師問：「目前的評分有參考評分標準嗎？」
#    ★ 原本把「老師寫的評分標準」和「系統內建的 A～G 防禦規則」
#      合併成一個數字，而防禦規則本身就約 2000 字（實測骨架 2074），
#      老師的標準被埋在裡面 —— 完全看不出來有沒有送出去。
#    ⇒ 拆開報。這是老師最需要知道的一件事。
ok('老師的評分標準 %d／系統防禦規則 %d' in _core_src,
   '★★★ 評分標準要和系統防禦規則分開報 —— 合在一起等於看不見')
# ⛔ 沒有評分標準 = AI 在沒有依據的情況下打分數。
#    那不是「比較寬鬆」，是每個學生拿到的標準都不一樣，而且完全無聲。
ok('沒有評分標準' in _core_src and 'rules or ""' in _core_src,
   '★★★ rules 是空的要大聲講 —— 那時分數不可信，不是寬鬆而已')
...
# ⛔⛔ 2026-09-08 清理報告的實際輸出：801/17 第 2 關：02.png、02.png
#    ★ 兩個**同名**檔 —— 不是補零問題。是「列出同名 → 丟垃圾桶 → 建立」
#      這三步不是不可分割的，而 Apps Script 的 doPost 可以同時跑好幾個執行。
ok('LockService.getScriptLock()' in _gs,
   '★★★ 證書寫入要序列化 —— 兩個請求同時進來會各建一個同名檔')
ok('lock = null;' in _gs and 'return replaceFile(folder, pad2' in _gs,
   '★★★ 拿不到鎖也要照做 —— 多一個重複檔遠比少一張證書輕微，'
   '而且清理工具撈得回來')
ok('MM/dd HH:mm:ss' in _gs,
   '★★★ 清理報告要印建立時間：同一秒＝併發、差好幾天＝重驗，'
   '兩者根因完全不同，沒有時間就只知道「有重複」不知道為什麼')
...
# ⛔⛔ 2026-09-07 老師：「積了 174 張，怎麼數字沒有減少？
#    但是資料夾檔案似乎變少了。」—— **老師是對的，數字才是錯的。**
#    工作者是「掃一次 → 把整批跑完 → 才再掃一次」，
#    而排隊快取只在每一輪開頭更新 ⇒ 174 張跑完要好幾個小時，
#    那個數字就凍在原地那麼久，看起來和真的卡死一模一樣。
_wl = _srv7[_srv7.index('def _temp_worker_loop'):][:6000]
ok('"_fid"' in _wl,
   '★★ 快取要帶檔案 id，才能精準移除處理完的那一張')
ok(_wl.count('_temp_queue_cache[:]') >= 2,
   '★★★ 每處理完一張就要從快取移掉 —— '
   '只在每輪開頭更新的話，數字會凍住好幾個小時')
_ql = _srv7[_srv7.index('def api_queue_list'):][:1200]
ok('!= "_fid"' in _ql,
   '★★ 回給前端之前要拿掉 _fid（那是雲端硬碟的檔案 id，沒必要送出去）')
