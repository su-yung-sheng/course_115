...
section('★★★ 關卡小卡：二星的人要找得到去程式設計的路');
{
  /* ⛔⛔ 老師 2026-09-17：「這個畫面中不是應該可以直接連到程式設計？因為只有二星」
     `both`＝流程圖過了 ＋ 程式 ≥2⭐，而 2⭐ 和 3⭐ 都算 both。
     原本「🐱 直接去程式設計」的條件帶著 `&& !both` ——
     於是學生拿到第 2 顆星的那一刻，通往程式設計的按鈕**剛好消失**。
     他想挑戰 3 星，畫面上一條路都沒有：
       點卡片 → 重排流程圖（不是他要的）
       🧜 流程圖代碼 → 純檢視（也不是）
     ★ 這是「獎勵把出口關掉」——最難發現的那種，因為它只發生在做得好的人身上。 */
  ok(!/open && done && !both\)\?`<span data-prog/.test(FC),
     '★★★ 「直接去程式設計」的條件**不可以**再帶 !both（二星的人正是最需要它的）');
  ok(/\$\{\(open && done\)\?`<span data-prog/.test(FC),
     '★★ 只要流程圖排對過就一直看得到這顆按鈕');
  ok(/both && st < 3\) \? '✅ 兩軌都完成了 —— 程式還有 3⭐/.test(FC),
     '★★★ 底部那一句要講「還差什麼」，不是只說「都完成了」');

  /* 按鈕文字的行為 —— 直接把那段三元運算跑起來，不要只比對字串。 */
  const lbl = (FC.match(/st===0 \? '🐱 直接去程式設計 →'[\s\S]*?: '🐱 再去程式設計'/) || [''])[0];
  ok(lbl.length > 0, '找得到按鈕文字那一段');
  if (lbl) {
    const f = new Function('st', 'return (' + lbl + ');');
    ok(/直接去程式設計/.test(f(0)), '0 星 → 「直接去程式設計」');
    ok(/挑戰 2 星/.test(f(1)), '1 星 → 「挑戰 2 星」');
    ok(/挑戰 3 星/.test(f(2)),
       '★★★ **2 星 → 「再傳一次，挑戰 3 星」**（老師回報的就是這一格）');
    ok(/再去程式設計/.test(f(3)) && !/挑戰/.test(f(3)),
       '★★ 3 星 → 不要再喊「挑戰」，AI 星數已經滿了');
  }
}
