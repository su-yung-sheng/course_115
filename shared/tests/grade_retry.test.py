...
# ⛔ ④y 2026-09-23 一天踩到三次同一個形狀：規則寫「A 或 B」，模型只認 A。
#    「回到起始位置**或**設定好初始狀態」→ 兩個模型都要求 motion_gotoxy
#    「碰到邊緣**或**碰到敵方戰車」→ flash 要求迴圈條件要含敵方戰車
#  ★ 老師心裡的寬容沒有寫進文字裡時，模型會挑最嚴的那個解讀。
ok("做到任何一種就算完成" in _prompt6 and "不可以要求兩者都有" in _prompt6,
   "★★★ 「A 或 B」要明講任一即可 —— 一天之內這個形狀出現三次")
...
section("④r 資產清單把模型「叫去」扣分（2026-09-23，第 4、5 關校正）")
# ⛔⛔ 這一組和前面幾組不一樣：**不是模型亂扣，是 prompt 自己叫它扣的。**
#    第 5 關（2-1-4）flash 100／haiku 65，haiku 的第 2 條逐字引用了
#    extract_asset_manifest 印出來的那句「資產清單明確警告：造型只有 1 個，
#    使用『切換造型』積木無實際效果」。
#    那段話是①（模型自己填 score）年代寫的，用意是防止「用了積木但沒有資產」
#    白白拿分；②之後分數從 100 往下扣，「不給分」＝「扣分」，於是變成
#    只要資產少就倒扣 —— 即使規則根本沒要求那個效果。
# ⚠️ 負面斷言前要先去掉註解行：上面那幾行註解就引用了舊句子，
#    不去掉的話「舊句子已經拿掉了」這種檢查會被自己的註解打成紅字。
#    （這個 repo 被這件事咬過不只一次。）
_nc = "\n".join(l for l in _src_key.split("\n") if not l.strip().startswith("#"))

ok("以此為最終事實依據" not in _nc and "必須依事實扣分" not in _nc,
   "★★★ 資產清單不可以再叫模型「依事實扣分」—— 事實是對的，"
   "但要不要扣分取決於**規則有沒有要求**，不取決於資產數量")
ok("不可給切換造型相關分數" not in _nc and "不可給切換背景相關分數" not in _nc,
   "★★★ 「不可給相關分數」在②之後等於「從 100 分裡扣掉」，"
   "這句話要拿掉 —— haiku 就是照著它扣了 15 分")
ok("只有當【老師的評分規則明確要求】" in _nc,
   "★★★ 要改成**有條件**的：規則有要求才算數")
ok(_nc.count("只有在評分規則**要求**") >= 3,
   "★★ 造型、背景、音效三個警告都要加上同一個條件，"
   "只改一個的話模型會從沒改的那個繞過去")

# ⛔ 第 4 關（2-1-3）haiku 扣 20 分：「成績單清單長度為 0（資產清單顯示：長度: 0），
#    導致迴圈永遠不會執行」。清單長度是**存檔當下的快照** ——
#    程式本來就是執行時才把分數加進去，存檔時是 0 才正常。
ok("存檔時 {list_len} 項" in _nc and "不是執行結果" in _nc,
   "★★★ 清單長度要標明是「存檔時」的快照 —— 標「長度: 0」會被讀成執行結果")

# ⛔ 第 5 關 haiku 的第 2 條還多錯一層：規則要求的是「**顯示結果的角色**會有
#    開場造型切換動畫」，它自己也寫了「Ball 角色的開場洗牌動畫正確實現」，
#    卻拿「抽獎員：造型 1 個」這則提醒把 15 分扣在抽獎員頭上。
#    ⇒ 資產提醒是逐角色的，要講明不可以跨角色套用。
ok("只針對**它所屬的那一個角色**" in _nc,
   "★★★ 資產提醒要講明是**逐角色**的 —— 不講的話模型會把 A 角色的資產不足，"
   "算到「規則其實是要求 B 角色做」的那一項上")
ok("是不是要求**這個角色**做的" in _nc,
   "★★ 要給一個扣分前的自我檢查：規則要求的是哪一個角色")

_p8 = _src_key[_src_key.index("🛡️【全域防禦規則"):]
ok("H. 存檔快照鐵律" in _p8,
   "★★★ 要有一條鐵律講清楚：存檔當下的清單內容不是執行結果")
ok("迴圈不會執行" in _p8 and "和迴圈設定的次數對不起來" in _p8,
   "★★★ 要把 haiku 實際用過的兩個扣分理由**逐字**列為禁止 —— "
   "抽象地說「不要看快照」沒有用")
ok("實際執行結果與預期不符" in _p8,
   "★★ 這是它包裝那個扣分的說法，也要一起堵")
ok("資產清單顯示長度為" in _p8,
   "★★ 要留一個**自我檢查的觸發語**：寫得出這句話就代表違反了")

# ⛔ 第 4 關 haiku 另外兩條（各扣 10 分）：總分／第幾項「初始化不在綠旗事件中，
#    而是在小老師被點擊事件中」。規則寫的是「開始計算前」，不是「在綠旗底下」。
ok("I. 初始化位置鐵律" in _p8 and "不限定要寫在" in _p8,
   "★★★ 「開始計算前」是**執行順序**，不是**寫在哪個帽子積木底下**")
ok("必須在綠旗事件中初始化" in _p8,
   "★★ 要留出口：老師真的想考這個時，寫進規則就算數")

section("④s 第 4 關第二輪：haiku 換了說法繞過鐵律 I（2026-09-23）")
# ⛔ H＋I 上線後重跑：flash 100／haiku 80。剩下的 20 分是同一條規則
#    換了兩個包裝：
#      ①「綠旗事件是程式啟動的標準入口，所以初始化應該寫在那裡」
#      ②「如果使用者不點擊角色就直接執行，總分會保留存檔時的值 2434」
#    ★ ② 同時踩到 H（拿存檔快照當證據）和第 6 條（以正常操作為準），
#      但它把三件事編在一起，任何一條單獨看都像沒被違反。
#    ⇒ 逐字堵。這個 repo 學到的事：抽象原則一定會被繞過，具體句型繞不過。
ok("綠旗事件是程式啟動的標準入口" in _p8,
   "★★★ 要堵住它把自己的寫作習慣當成規則的那個說法")
ok("會保留存檔時的值" in _p8,
   "★★★ 要把「變數會保留存檔時的值 N」當成觸發語 —— "
   "那句話同時違反 H（存檔快照）和第 6 條（正常操作）")
ok("不點就不會開始計算" in _p8,
   "★★ 要正面說明為什麼那不算：不點角色就不會開始計算，那不是正常操作")

# ⛔ 同一份回應裡它還放了一條「【1.清單初始化】…完成清單初始化（扣 0 分）」。
#    加總沒問題，但學生的扣分明細會多出一條看起來像扣分的字。
_z = {"score": 1, "bonus_points": 0,
      "deductions": [{"rule": "1.清單初始化", "points": 0,  "why": "完成初始化"},
                     {"rule": "2.總分初始化", "points": 10, "why": "位置不對"}]}
_zsrc, _ = core.apply_deductions(_z)
ok(_zsrc == "backend" and _z["score"] == 90,
   "★ 扣 0 分的項目照樣參與加總（加 0），分數不受影響　←　%r" % _z["score"])
ok("扣 0 分" not in _z["deducted_items"],
   "★★★ 扣 0 分的項目**不可以印進扣分明細** —— "
   "學生看到「（扣 0 分）」只會以為自己被扣了分")
ok(_z["deducted_items"].startswith("1. 【2.總分初始化】"),
   "★★ 過濾之後編號要重新從 1 開始，不可以跳號　←　%r" % _z["deducted_items"])
_z2 = {"score": 1, "bonus_points": 0,
       "deductions": [{"rule": "x", "points": 0, "why": "沒問題"}]}
core.apply_deductions(_z2)
ok(_z2["deducted_items"] == "無" and _z2["score"] == 100,
   "★★ 全部都是 0 分時要顯示「無」，不是一條空的明細　←　%r" % _z2["deducted_items"])


section("④t 第 6 關：觸發語被換掉三個字就失效了（2026-09-23）")
# ⛔⛔ haiku 扣的第 1 條原文：
#      「**雖然最終效果上**『撲克牌』確實會隱藏，但邏輯分散在兩個角色，
#        不符合『當綠旗被點擊時』的統一初始化要求。」
#    第 6 條早就寫了觸發語「雖然最終效果達成，但是…」——
#    它寫的是「雖然最終效果**上**…確實…」，**字面對不上，就繞過去了**。
#  ★ 教訓：觸發語要認**句型**（先承認做到了、再換一個理由扣分），
#    不能只列一個句子。逐字列出來是為了具體，不是為了字面比對。
ok("雖然最終效果達成" in _p7 and "雖然最終效果上確實" in _p7,
   "★★★ 觸發語要涵蓋它實際寫過的變體 —— 只列一句的話，換三個字就失效")
ok("認的是**句型**" in _p7,
   "★★★ 要明講認的是句型不是字面，否則永遠在追它的新說法")

# ⛔ 第 2 條：初始化分散在各角色自己的綠旗事件裡。
#    這是 Scratch **正常且正確**的寫法（每個角色都有自己的綠旗事件）。
ok("哪一個角色" in _p8 and "邏輯分散在兩個角色" in _p8,
   "★★★ 鐵律 I 要涵蓋「寫在哪一個角色身上」—— "
   "原本只講「寫在哪一個帽子積木底下」，它換成「角色」就繞過去了")

# ⛔ 第 3 條是純臆測：它自己寫「這在邏輯上應該可行」「Scratch 會自動轉換」，
#    然後用「在嚴格的型別檢查下**可能**失敗」扣 5 分。
ok("J. 臆測禁止鐵律" in _p8,
   "★★★ 要有一條鐵律：扣分必須指出**確定會發生**的錯誤，不可以用臆測")
ok("可能失敗" in _p8 and "在嚴格的型別檢查下" in _p8,
   "★★★ 要逐字堵它用過的臆測句")
# ⚠️ 引號裡的觸發語**不可以被換行切斷**：這一條第一次就是紅的，
#    因為 prompt 裡寫成「Scratch 會自動\n     轉換」，字串裡夾了換行和縮排，
#    模型讀得懂，但這裡的比對（和模型的字面比對）對不上。
ok("應該可行" in _p8 and "會自動轉換" in _p8,
   "★★★ 要把它自己寫的肯定句當成自我檢查：寫了「應該可行」就不可以再扣")
ok("沒有驗證" in _p8,
   "★★ 「程式沒有驗證 X 是否成功」是防呆的新包裝，一併堵掉")





section("④w 第 7 關：全域變數被要求每個角色各初始化一次（2026-09-23）")
# ⛔ haiku 扣 10 分：「蝠角色的綠旗事件中沒有初始化全域變數『被點幾下』，
#    導致全域變數初始化不完整。」
#  ★ 全域變數是所有角色共用的同一個，有一個角色初始化就夠了。
#    真的每個角色都寫一次，反而會互相覆蓋 —— 它要求的是**錯的寫法**。
#  ⇒ 鐵律 I 原本只講「寫在哪一個角色身上不限」，沒講「不必每個角色都寫」。
ok("全域變數只要有一個角色初始化就算完成" in _p8,
   "★★★ 要正面寫出判準，不是只禁止 —— 只說「不可以扣」時它會自己定義什麼叫完整")
ok("初始化不完整" in _p8,
   "★★★ 要逐字堵它用過的說法（「初始化不完整」）")
ok("互相覆蓋" in _p8,
   "★★ 要講出**為什麼**它要求的那個寫法是錯的 —— "
...
section("④x 第 8 關：拿存檔時的變數值扣分，理由是「學生理解不夠清晰」（2026-09-23）")
# ⛔ haiku 扣 5 分：「敵方戰車2 的『打到幾次』變數**定義時**初始值為 2（而非 0），
#    **雖然在綠旗事件中被重設為 0**，但這是不必要的錯誤初始化。
#    更重要的是，這表示**學生對變數初始化的理解不夠清晰**。」
#  ★ 一條扣分裡三個問題：拿存檔快照當事實（H）、先承認再扣分（第 6 條句型）、
#    理由是對學生的推測而不是程式事實。
#  ⚠️ 審查層擋不到這一條 —— 它的句型是「雖然…被重設為 0，但…」，
#     而「雖然…但…」太常出現在**正當**的扣分裡（「雖然有用迴圈，但次數寫死」），
#     放進審查層會誤擋。⇒ 這一類只能在條款層逐字堵。
ok("定義時" in _p8 and "不必要的錯誤初始化" in _p8,
   "★★★ 要逐字堵「定義時的初始值不是 0，是不必要的錯誤初始化」")
ok("完全不重要" in _p8,
   "★★ 要正面講：存檔時變數停在什麼值不重要，程式跑起來會重設就對了")
ok("理解不夠清晰" in _p8,
   "★★★ 要堵住「這表示學生對…的理解不夠清晰」")
ok("程式做了什麼" in _p8,
   "★★ 要給正面判準：扣分的理由必須是程式做了什麼，不是對學生的推測")

# ⛔ 第 8 關重跑（④x2）：同一條扣分，讓步句從「雖然在綠旗事件中被重設為 0」
#    換成「雖然**最終**會被重設為 0」，分數從 5 分改成 3 分 —— 它在微調說法。
ok("雖然**最終**會被重設為 0" in _p8 and "不是句子的寫法" in _p8,
   "★★★ 讓步句的變體也要涵蓋，並講明認的是**事實**不是句子的寫法")

# ⛔ 同一輪新出現：「四個方向鍵各自獨立運行在不同的綠旗迴圈中，
#    同時按上和左會不斷切換方向而無法斜向移動…（扣 10 分）」
#  ★ 規則只要求「用方向鍵控制移動」。斜向移動、多鍵同時按是進階手感，
#    規則沒寫就不是要求 —— 這是第 6 條「正常操作」的延伸。
ok("同時按下多個方向鍵會衝突" in _p7 and "無法斜向移動" in _p7,
   "★★★ 要逐字堵「多鍵衝突／無法斜向移動」這個扣分理由")
ok("每一個按鍵各自按下去會動" in _p7,
   "★★ 要給正面判準（每個鍵各自能動就算完成），不是只列禁止事項")

# ⛔ ④x3 第 8 關第三輪（後端確認是 da34f023，條款都在線上，它還是扣）：
#    「四個迴圈…互相搶奪控制權…會不斷抖動…此實現方式導致四個方向都無法正常控制」
#    「見資產清單『[定義/區域] 打到幾次 = 2』…定義時不應該設為 2」
#  ★ 兩條都不是「換句話說」的問題，是**我們給它看的東西有漏**：
#    ① 清單長度早就標「存檔時 N 項」了，變數的值卻還是光禿禿一個「= 2」。
#    ② 它在描述「執行起來會怎樣」—— 但它從來沒有執行過這個程式。
_nc2 = "\n".join(l for l in _src_key.split("\n") if not l.strip().startswith("#"))
ok("存檔當下的值，不是執行結果" in _nc2,
   "★★★ 變數的值也要標明是存檔快照 —— 只標清單、不標變數，它就從變數那邊扣")
ok("你沒有執行過這個程式" in _p8,
   "★★★ 要點破它在推論而不是觀察：模型看不到程式跑起來的樣子")
ok("會不斷抖動" in _p8 and "互相搶奪控制權" in _p8,
   "★★★ 要逐字堵它宣稱過的執行現象")
ok("國中課堂教的標準寫法" in _p8,
   "★★ 要正面說明那個寫法沒問題 —— 只說「不可以扣」時它會再發明一個缺陷")
...
section("④z2 第 9 關第二輪：鐵律寫了也沒用，改用兩帖老藥（2026-09-24）")
# ⛔ 條款（鐵律 J 的分身那一條）已經在線上，haiku 還是扣 30 分，
#    而且講得更細：「區域變數只在該角色的特定事件內有效，
#    分身產生時無法正確繼承父角色在綠旗事件中修改的值」。
# ★ 今天兩次真正有效的修法都不是加禁令：
#   ① 把事實寫在它**讀得到的地方**（虛擬碼裡），
#   ② 後端直接**不接受**那種輸出。
#   ⇒ 這次兩帖一起下。
_nc4 = "\n".join(l for l in _src_key.split("\n") if not l.strip().startswith("#"))
ok("每個分身\n" in _nc4 or "每個分身" in _nc4,
   "★★★ 虛擬碼裡要在區域變數底下直接講明「建立分身時每個分身各複製一份」")
# ⚠️ 2026-09-24 這句話被 ④z5 改寫了（從「只講變數」擴成「整個狀態」）——
#    斷言跟著改成新的結論句，但要檢查的事情沒變：**結論要講死**，
#    不可以只描述機制、讓它自己推。
ok("每個分身有自己的編號" in _nc4,
   "★★ 要把結論講死，不要留給它推論")

_c = {"score": 1, "bonus_points": 0, "deductions": [
        {"rule": "二、分身建立與隱藏", "points": 30,
         "why": "區域變數只在該角色的特定事件內有效，分身產生時無法正確繼承"
                "父角色在綠旗事件中修改的值。"}]}
core.apply_deductions(_c)
ok(_c["score"] == 100 and _c["deducted_items"] == "無",
   "★★★ 「分身無法繼承區域變數」是事實錯誤，審查層要擋掉　←　%s 分" % _c["score"])

# ⚠️⚠️ 反面：全域變數被所有分身共用是**真的**，那種扣分不可以被擋。
_c2 = {"score": 1, "bonus_points": 0, "deductions": [
        {"rule": "二、分身建立與隱藏", "points": 10,
         "why": "編號用全域變數，所有分身共用同一個，分身無法取得各自的編號"}]}
core.apply_deductions(_c2)
ok(_c2["score"] == 90 and not _c2.get("deduction_vetoes"),
   "★★★ 全域變數共用是真的 Scratch 行為，這種扣分一條都不可以被擋　←　%s 分"
   % _c2["score"])


section("④z3 加分題的判定標準（2026-09-24）")
# ⛔ 十關的扣分全部收斂之後，剩下的差距**都在加分題**，而且方向相反：
#      第 9 關 flash 0 ／ haiku +10　（「分身數量用變數控制」）
#      第 10 關 flash +5 ／ haiku 0 　（「螞蟻移動看起來更自然」）
# ★ 這比扣分更危險：加分題會影響星數（85 分的作品 +10 就從 2⭐ 變 3⭐），
#   而畫面上**不會列出「為什麼沒給你加分」**⇒ 判定不一致時沒有任何徵兆。
_p3 = _p7[:_p7.index("4. 抄襲")]
ok("指出具體的積木" in _p3,
   "★★★ 加分要有客觀門檻：指得出是哪幾塊積木才給")
ok("一律不加分" in _p3,
   "★★★ 不確定時要有一個**固定**的預設（不加），兩個模型才會一致")
ok("比較有創意" in _p3 or "看起來比較自然" in _p3,
   "★★ 要逐字點名主觀判斷的說法")
ok("bonus_points 填 0" in _p3,
   "★★★ 要講明「不加分」怎麼表達 —— 不講的話它可能又寫進 deductions 變成倒扣")


section("④z4 自己宣告「此扣分撤銷」，然後照樣留 5 分（2026-09-24）")
# ⛔ 第 10 關 haiku 的扣分原文結尾：
#      「…這個要求已滿足。…此項應給分。**此扣分撤銷**。（扣 5 分）」
#    它自己撤銷了，points 還是 5。
#  ★ 這是 2026-09-19「扣分調整：僅扣…3 分」的同一家族 ——
#    模型會在文字裡改主意，但不會回頭改數字。
for _w in ["此項應給分。此扣分撤銷。",
           "扣分調整：僅扣初始化拼寫錯誤 3 分",
           "檢查後此項應給分"]:
    _r = {"score": 1, "bonus_points": 0,
          "deductions": [{"rule": "x", "points": 5, "why": _w}]}
    core.apply_deductions(_r)
    ok(_r["score"] == 100 and _r["deducted_items"] == "無",
       "★★★ 要擋下自己撤銷的扣分：%s　←　%s 分" % (_w, _r["score"]))

# ⚠️⚠️ 反面：「不應給分」是**正當**的扣分寫法，不可以被 r"(?<!不)應給分" 擋掉。
_r = {"score": 1, "bonus_points": 0,
      "deductions": [{"rule": "x", "points": 5,
                      "why": "空的條件判斷，功能未實作，此項不應給分"}]}
core.apply_deductions(_r)
ok(_r["score"] == 95 and not _r.get("deduction_vetoes"),
   "★★★ 「不應給分」是正當扣分，一個字之差不可以擋錯　←　%s 分" % _r["score"])


section("④z5 同一個分身誤解換到「位置」上（2026-09-24）")
# ⛔ 第 10 關：它自己寫「每次都執行隨機移動**後才**建立分身」，
#    也寫了「分身會繼承該位置」，結論卻是
#    「所有分身會在同一個位置開始活動」（扣 5 分）。
#  ★ 第 9 關是區域變數，這一關是位置 —— 同一個誤解換了對象。
#    ⇒ 虛擬碼那一段不能只講變數，要講**分身複製的是當下的整個狀態**，
#      而且只要角色用到分身積木就要印（不是只有有區域變數時才印）。
_nc5 = "\n".join(l for l in _src_key.split("\n") if not l.strip().startswith("#"))
ok("當下的整個狀態" in _nc5 and "位置、造型、大小、方向" in _nc5,
   "★★★ 要講清楚分身複製的是整個狀態，不是只有變數")
ok("先隨機移動、再建立分身" in _nc5,
   "★★★ 要把這個寫法直接舉例出來 —— 它每次都是在這個形狀上出錯")
ok("_has_clone" in _nc5 and "variables or _has_clone" in _nc5,
   "★★ 沒有區域變數但有用分身的角色也要印 —— "
   "第 10 關的螞蟻就可能是這種，只看 variables 會漏掉")
ok("所有分身會在同一個位置開始活動" in _p8,
   "★★ 鐵律 J 也要逐字堵這句結論")


section("④z6 分身位置：換三次說法之後交給審查層（2026-09-24）")
# ⛔ 同一條扣分第三輪，理由每次都長這樣：
#      「分身建立前執行 motion_goto(_random_)，但此時原始螞蟻**已經移動到隨機位置**。
#        分身建立時會**繼承該位置**，導致所有 5 隻分身都在同一個位置」
#    前提（移動過了、會繼承）和結論（都在同一個位置）自相矛盾。
# ⚠️⚠️ 不可以直接擋結論句「所有分身都在同一個位置」——
#    學生真的把隨機移動寫在迴圈**外面**時，那是一條正當的扣分。
#    ⇒ 只擋「自己承認移動過了」的那兩句。
_c3 = {"score": 1, "bonus_points": 0, "deductions": [
        {"rule": "二、螞蟻大軍出動", "points": 5,
         "why": "分身建立前執行 motion_goto(_random_)，但此時原始螞蟻已經移動到"
                "隨機位置。分身建立時會繼承該位置，導致所有 5 隻分身都在同一個位置。"}]}
core.apply_deductions(_c3)
ok(_c3["score"] == 100 and _c3["deducted_items"] == "無",
   "★★★ 前提自己推翻結論的扣分要擋掉　←　%s 分" % _c3["score"])

# ⚠️⚠️ 反面：學生真的沒在迴圈裡隨機移動 ⇒ 正當扣分，不可以擋。
_c4 = {"score": 1, "bonus_points": 0, "deductions": [
        {"rule": "二、螞蟻大軍出動", "points": 5,
         "why": "隨機移動寫在重複迴圈外面，只執行一次，所有分身都在同一個位置"}]}
core.apply_deductions(_c4)
ok(_c4["score"] == 95 and not _c4.get("deduction_vetoes"),
   "★★★ 隨機移動真的在迴圈外時是正當扣分，一條都不可以擋　←　%s 分" % _c4["score"])
