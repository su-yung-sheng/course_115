...
section('題庫本身');
/* ⚠️ 2026-08-12 之前這裡只檢查 4-2-1／4-2-2／4-2-3 三關 ——
      那是當時只有那三關有題庫。後來十關陸續補上題庫，
      新加的那七關就完全沒有被這一段檢查過。
   ★ 改成「凡是有 quiz 的關卡都檢查」：以後補新關卡不必記得回來加名字，
     忘了加也不會靜靜地少一份保護。 */
const QUIZ_IDS = Object.keys(W.BLOCK_LEVELS).filter(id => (W.BLOCK_LEVELS[id].quiz || []).length);
ok(QUIZ_IDS.length >= 9, '★ 有題庫的關卡共 ' + QUIZ_IDS.length + ' 關，全部都要跑下面這幾條');
QUIZ_IDS.forEach(id => {
  const bank = Q(id);
...
/* ⚠️ 這一條原本是「6-1-1 沒有題庫」——
   當時它只有拆解與手動排序，題庫是刻意不辦的（寧可不辦，也不要湊題目）。
   2026-08-12 補上六題之後那個前提不存在了。
   ★ 但那條規則本身沒有消失，只是換個講法：
     有題庫就要夠抽（N_TOTAL 題以上），不到就是湊出來的 —— 上面那圈已經在釘。
   ⇒ 這裡改成釘「十關都有題庫了」，少一關就變紅。 */
ok(QUIZ_IDS.length === 10,
   '★ 十關都有概念檢測題庫了（實得 ' + QUIZ_IDS.length + ' 關）');
{
  /* 第 5 關和第 6 關要分工：第 5 關問「人怎麼做」，第 6 關問「電腦怎麼做」。
     ⚠️ 兩關問同一批東西的話，學生第二次會照抄第一次的答案 ——
        而那正好讓「從觀念到程式」那一步被跳過去。

     ⚠️⚠️ 2026-08-17 改過一次：原本第二條釘的是
          「第 6 關的題目要出現『變數』或『資料位置』」。
        那天老師退回第 4 關的題目：概念檢測排在程式拼圖**之前**，
        題幹用程式用語問，等於要學生先在腦中把程式拼出來。
        ⇒ 現在的規則是：題幹講做法，程式用語留給 hint／why（答完之後的解說）。
          那條斷言正好在要求相反的事，所以換掉。
        ★ 分工本身沒有消失 —— 只是改成用**問的內容**來釘，不用字眼釘。
          （見 shared/tests/concept.test.js） */
  const j5 = JSON.stringify(W.BLOCK_LEVELS['6-1-1'].quiz);
  ok(!/資料位置|副程式|函式積木|迴圈/.test(j5),
     '★ 第 5 關的題目不碰程式用語（資料位置／副程式／迴圈）—— 那是第 6 關的事');
  const q6 = W.BLOCK_LEVELS['6-2-1'].quiz.map(function (q) { return q.q; }).join(' ');
  ok(/假設/.test(q6) && /矮/.test(q6) && /下一個/.test(q6),
     '★★ 第 6 關才問「電腦怎麼做」的三個動作：先假設一個、遇到更矮的換人、往下一個');
  ok(!/變數|資料位置/.test(q6),
     '★★ 而且是用做法問，不是用程式用語問（題幹裡不出現「變數」「資料位置」）');
}
...
/* ── ★ 不值得覆核的就不要送 ───────────────────────
   ...
   六個字又一個概念都沒沾到的答案，沒有東西可以撿 ——
   送出去只是白花額度，而額度是全班共用的。

   ⚠️ 但條件必須是「太短**而且**沒沾到任何概念」，兩個都要成立。
      只看字數的話會殺掉「省事啊」這種三個字卻講到重點的答案，
      而 answer.js 的規矩正好相反：講到概念就不套用字數限制。
      這兩套規矩要是打架，學生會遇到「明明答對卻沒被撿回來」，
      而畫面上完全看不出原因。 */
section('★ 太短又什麼都沒講到 → 不送 AI（省額度）');
{
  ...
  /* 短、但沾到概念 → 一定要送。
     自己造一題 full:2 的，讓短答案只命中一群 —— 那正是
     「他懂一半、說法又不在收錄裡」最可能發生的形狀。 */
  const it = {
    q: '測試題', min: 20, full: 2,
    need: [
      { name: '甲概念', any: ['甲甲甲', '甲一', '甲二'] },
      { name: '乙概念', any: ['乙乙乙', '乙一', '乙二'] }
    ],
    hint: '無', why: '無'
  };
  sent = null;
