...
section('★★ 兩段走得完（老師 2026-08-18：實作體驗太弱了）');
{
  ...
  ok(/不是「挑最便宜的」/.test(v.txt()),
     '★★ 而且直說這不是挑最便宜的');
  ok(B.PLANS.every(p => p.tag), '★ 每個方案都有一個小標（保證價／看運氣／最省）');
  eq(B.PLANS.map(p => p.tag), ['保證價', '看運氣', '最省'],
     '★★ 最貴的那個有存在的理由：**保證價**（不管多亂都不會更糟）');
  /* ⚠️ 「看運氣」那個一定要講出最壞情況 ——
     只寫平均值的話，學生會以為插入排序永遠比選擇排序省，那是錯的。 */
  ok(/最壞會到/.test(B.PLANS[1].sub),
     '★★ 插入排序（很亂）要講明**最壞會到**選擇排序那個數');
