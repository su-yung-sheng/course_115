...
section('★★ 兩段走得完（老師 2026-08-18：實作體驗太弱了）');
{
  ...
  eq(fees, [4950, 2475, 99], '★★ 100 筆的三種**比較**次數（選擇／插入很亂／插入接近排好）');

  /* ── 搬動：帳單漏掉的另一半（老師 2026-08-18）─────────────
     「但是以數字來說還是最大，這很難令人信服，次數之外還有什麼背後的成本考量？
       不然學生會說都用插入排序就好了。」
     ⚠️ 老師是對的，而且我上一版的「保證價」**站不住**：
        選擇排序保證 4,950，而插入排序**最壞也才** 4,950 ——
        同樣的價錢卻常常更便宜 ⇒ 那當然是「都用插入排序就好」。
        ★ 病根是帳單只算了比較。
     ★ 漏掉的是搬動：選擇排序每回合只搬一次（n−1），
       插入排序每插一張牌前面的全要往右挪（約 n²/4）—— 100 筆差 25 倍。 */
  eq(B.PLANS.map(p => B.movesOf(100, p.key)), [99, 2475, 0],
     '★★ 搬動次數：選擇 99、插入（亂）2,475、插入（接近排好）0');
  ok(B.movesOf(100, 'insRand') / B.movesOf(100, 'sel') > 20,
     '★★ 插入排序的搬動是選擇排序的 ' +
     Math.round(B.movesOf(100, 'insRand') / B.movesOf(100, 'sel')) +
     ' 倍 —— 那才是選擇排序存在的理由');

  /* ★★ 「一筆資料有多大」決定搬一次值幾次比較 —— 切過去要真的翻盤 */
  eq(B.SIZE_KINDS.map(k => k.mul), [1, 10], '★ 數字 ×1、照片 ×10');
  const costNum = B.PLANS.map(p => B.sortCost(100, p.key, 'num'));
  const costPhoto = B.PLANS.map(p => B.sortCost(100, p.key, 'photo'));
  ok(costNum[0] > costNum[1],
     '★ 資料是**數字**時：插入排序（' + costNum[1] + '）還是比選擇（' +
     costNum[0] + '）省一點');
  ok(costPhoto[0] < costPhoto[1],
     '★★ 換成**照片**就翻盤了：選擇 ' + costPhoto[0] + ' vs 插入 ' + costPhoto[1] +
     '（省 ' + Math.round(costPhoto[1] / costPhoto[0]) + ' 倍）');
  eq(B.PLANS.map(p => B.breakEvenBy(100, p.key, 'num')), [55, 54, 2],
     '★★ 分界點（數字）：55 → 54 → 2');
  eq(B.PLANS.map(p => B.breakEvenBy(100, p.key, 'photo')), [64, 293, 2],
     '★★ 分界點（照片）：64 → 293 → 2　←　插入排序整個變貴了');

  /* ── 三個選項不可以有「永遠不該選」的那一個（老師 2026-08-18）──
     「如果選擇排序法都是相同金額，為什麼要選這個選項？學生會有疑問吧？」
     ⚠️ 這是**設計錯了**，不是文案問題：
        前一版寫成「你要用哪一種排序法」→ 那就是一道「挑最便宜的」的題目，
        而選擇排序永遠最貴 ⇒ 它是一個永遠不該選的選項 ⇒ 那個選擇是假的。
     ★ 真正的答案：不是挑最便宜的，是**看你手上的資料長什麼樣**。
       而選擇排序的價值是「不會更糟」—— 最壞情況的保證。 */
  ok(/哪一種情況/.test(v.txt()),
     '★★ 問的是「你手上的資料是哪一種情況」，不是「你要用哪一種排序法」');
  ok(/不是「挑最便宜的」/.test(v.txt()),
     '★★ 而且直說這不是挑最便宜的');
  ok(B.PLANS.every(p => p.tag), '★ 每個方案都有一個小標');
  eq(B.PLANS.map(p => p.tag), ['很少搬動', '比較少但搬很多', '最省'],
     '★★ 最貴的那個有存在的理由，而且是**真的**理由：搬動少');
  /* ⚠️ 上一版寫「保證價」—— 那是假的理由（保證的價錢和對方最壞情況一樣）。
     ★ 小標要講**它贏在哪裡**，不是講「它不會更糟」。 */
  ok(!/保證價/.test(B.PLANS.map(p => p.tag).join('')),
     '★★ 不再用「保證價」當理由（保證的價錢和插入的最壞情況一樣，說服不了人）');
  ok(/往右挪/.test(B.PLANS[1].sub),
     '★★ 插入排序那張要講明「前面的全要往右挪」—— 搬動從哪裡來');
  ok(/只搬一次/.test(B.PLANS[0].sub),
     '★★ 選擇排序那張要講明「每一回合只搬一次」');
  ok(/前提是資料/.test(B.PLANS[2].sub),
     '★★ 最省的那個要講明前提（那不是你能挑的，是資料本來就長那樣）');
  /* 選完之後要回一句**它自己的**話 —— 三種都回同一句的話，那個選擇就白選了 */
  const says = [];
  ['sel', 'insRand', 'insSorted'].forEach(k => {
    v.pick(k);
    says.push((v.host.querySelector('.bc-msg') || {}).textContent || '');
    const back = v.host.querySelector('[data-pick=""]');
    if (back) back.onclick();
  });
  ok(new Set(says).size === 3, '★★ 三種方案各回一句不同的話');
  ok(/保證價/.test(says[0]), '　 選最貴的 → 講「保證價」');
  ok(/平均/.test(says[1]), '　 選插入（很亂）→ 提醒那是平均');
  ok(/前提/.test(says[2]), '　 選最省的 → 提醒前提');
  v.pick('sel');
  ...
  /* ── 帳單要**兩行排序費**（老師 2026-08-18）────────────
     只算比較的話，插入排序永遠不比選擇排序差 ——
     那學生當然會說「都用插入排序就好」。搬動那一行才是選擇排序存在的理由。 */
  ok(/排序 · 比較/.test(billTxt) && /排序 · 搬動/.test(billTxt),
     '★★ 排序費拆成「比較」和「搬動」兩行');
  ok(/一筆資料是/.test(v.txt()), '★★ 而且有「一筆資料是數字／照片」的切換');
  const kindBtn = k => { const el = v.host.querySelector('[data-kind="' + k + '"]'); if (el) el.onclick(); };
  /* ★★ 切到照片要**當場翻盤** —— 看得到，而不是被告知 */
  /* ⚠️ 要比的是**搬動那一行**，不是總計 ——
     總計還含著查詢費（k 是前面留下來的），會把效果稀釋掉。 */
  const moveFee = () => {
    const rows = [...v.host.querySelectorAll('.bc-bill.sorted .br')];
    const r = rows.filter(x => /搬動/.test(x.textContent))[0];
    return r ? Number(r.textContent.replace(/[^\d]/g, '')) : -1;
  };
  v.pick('insRand');
  const numMv = moveFee();
  kindBtn('photo');
  const photoMv = moveFee();
  eq(photoMv, numMv * 10,
     '★★ 換成照片，插入排序的搬動費 ×10（' + numMv + ' → ' + photoMv + '）');
  ok(/最省的就換人了|換人/.test(v.txt()),
     '★★ 而且直說「搬動一變貴，最省的就換人了」');
  kindBtn('num');
  v.pick('sel');
  ...
  ok(goal().textContent.indexOf(String(B.breakEvenBy(v.s().n, v.s().pick, 'num'))) < 0,
