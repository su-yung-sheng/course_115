...
}
{
  ...
  /* ⚠️ 2026-08-17：第 5 關**沒有實作測試**了。
     它是觀念導入，沒有程式作品要交（見 GRADING.GATE.NO_UPLOAD）——
     最後一步換成 100 人的「實作體驗」。
     ★ 要釘的還是同一件事：**最後一定要有一步**，不然畫面會空掉。 */
  ok(s.some(x => x.indexOf('實作體驗') >= 0),
     '★★ 第 5 關的最後一步是「實作體驗」（不是實作測試 —— 這一關沒有作品要交）');
  ok(s.every(x => x.indexOf('實作測試') < 0),
     '★ 而且**不該**再出現「實作測試」—— 那會叫學生去交一個不存在的作業');
}
{
