...
section('★ 說明講「橘框」，那一格就要真的是橘的');
{
  /* .sl-cell.card 是插入排序那張「新牌」。
     ⚠️ 2026-08-24 修：原本寫的是 `r > 200 && g > 90 && b < 90`——
        那是**照著 #f97316 這一個值反推出來的門檻**，不是「橘色」的定義。
        邊框後來為了和橘底拉出對比而調深成 #c2410c（R194 G65 B12），
        肉眼明明還是橘的，測試卻因為 r 差 6 就紅了 ——
        紅在「顏色沒對」上，其實顏色是對的。
     ⇒ 改成看**色相**：橘色的定義是紅通道最大、色相落在 15～45 度。
        深橘、淺橘都算；紅（0 度）和黃（60 度附近）會被擋下來 —— 那才是真的講錯色。 */
  ...
  if (m) {
    ...
    const hue = d === 0 ? -1 : (max === r ? ((g - b) / d * 60 + 360) % 360
                             : max === g ? (b - r) / d * 60 + 120
                                         : (r - g) / d * 60 + 240);
    ok(max === r && d > 100 && hue >= 15 && hue <= 45,
...
section('★ 資料的單位一律叫「項」，不叫「格」');
{
  ...
  const code = SRC.search.replace(/\/\*[\s\S]*?\*\//g, ' ').replace(/^\s*\/\/[^\n]*/gm, ' ')
    /* ⚠️ 唯一的例外：資料太多時畫不下，會標「一格 = 1,000 項」。
       那一句的**用途正好就是講清楚「一格不等於一項」**——
       把它一起擋掉的話，就沒有辦法解釋比例尺了。 */
    .replace(/一格 = /g, ' ');
