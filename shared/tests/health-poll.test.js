...
for (const term of ['11501', '11502']) {
  ...
  /* ⚠️⚠️ 「掃到 0 筆」和「後端根本掃不到」不可以顯示成同一句 ——
     2026-09-03 老師實測時就對著「目前沒有人在排隊」等到天荒地老。
     ⚠️ 但也不可以一次失敗就跳紅字：GAS 偶爾抖一下，下一輪就好了。
     ★ 和健康檢查的 MISS_LIMIT 同一個原則 —— 連續失敗才算壞。 */
  ok(/scan\.fails/.test(CODE) && />=\s*3/.test(CODE),
     '★★ 要連續三次掃描失敗才顯示「後端看不到暫存區」');
  ok(/後端看不到暫存區/.test(SRC) && /不會不見/.test(SRC),
     '★★ 真的壞掉時要講「你的截圖已經在雲端、不會不見」');

  /* ⚠️⚠️ 2026-09-03 老師：「所以還是要一直開著才會處理？這樣就沒有達到
     學生下課也能離開的目標了。」—— 他是對的，那句「不要重開視窗」
     和整套設計的目標剛好相反。成績是後端寫 Firestore 的，
     跟這個視窗開不開無關，下次登入還會自動補記。
     ★ 這一條盯著那句話不要再長回來。 */
  /* ⚠️ 用 CODE（已去掉註解）而不是 SRC —— 註解裡會引用那句話來說明
     「為什麼把它拿掉」，拿 SRC 比對會把說明本身判成違規。
     要釘的是**學生真的看得到的字**。 */
  ok(!/不要重開視窗/.test(CODE),
     '★★★ 不可以叫學生「不要重開視窗」—— 傳完就能走才是這套架構的目的');
  /* ⚠️⚠️ 2026-09-03 老師：「怕按下就直接關閉，確認進入認證排隊後，
     應該要有訊息提醒可以關閉視窗。」
     ★ 關鍵是**時機**：按下送出到 GAS 確認收到之間還有幾秒，
       那段時間關掉圖是真的會掉的 ⇒ 兩段訊息不可以合成一段。 */
  ok(/isProcessing && !myUpName/.test(CODE) && /isProcessing && myUpName/.test(CODE),
     '★★★ 「上傳中」和「已進排隊」要分成兩段（時機不同，能不能關也不同）');
  ok(/這幾秒請先不要關掉視窗/.test(SRC),
     '★★★ 還在上傳時要叫他先別關（這時候關真的會掉圖）');
  ok(/可以關掉視窗了/.test(SRC) && /不會不見/.test(SRC),
     '★★★ 確認排進佇列後要大聲說「可以關掉」');
  ok(/可以關視窗/.test(SRC),
     '★★ 按鈕文字也要跟著改，不要一直寫「處理中」');
...
/* ═══════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════ */
/* ⚠️⚠️ 這一節的規則 2026-09-08 改了，改法本身要先講清楚。
   ★ 2026-09-03 曾經把「估計還要幾分鐘」**整個拿掉**，理由是：
     那個數字是估的，而估低了的後果特別壞 —— 說「約 1 分鐘」卻等了
     3 分鐘，學生會以為系統壞掉而重按，重按又讓後端更擠。
   ★ 2026-09-08 老師要求「一併處理」等待時間的說法。重新想過之後：
     那個理由對的是**估法**，不是「報不報」。完全不講也有代價 ——
     清單上寫「27 張等待辨識」，學生分不出那是兩分鐘還是二十分鐘，
     一樣會一直重整（而重整正好發生在後端最忙的時候）。
   ⇒ 現在允許報時間，但**三個條件缺一不可**，這一節守的就是這三條：
       ① 秒數只能來自後端實測（ocrAvg），不可以寫死
       ② 一律走 shared/waittime.js（半分鐘一格、一律往上，不會低估）
       ③ 問不到實測值時（ocrAvg 是 0）就閉嘴，只講張數
   ⚠️ 排隊清單仍然是主角：清單是事實本身，時間只是輔助。 */
for (const term of ['11501', '11502']) {
  section('★★ ' + term + '：等待時間只能用實測值、而且不可以低估');
  const SRC = fs.readFileSync(path.join(ROOT, term, 'thinking.html'), 'utf8');
  const CODE = SRC.replace(/\/\*[\s\S]*?\*\//g, ' ');

  ok(/WaitTime\.fmt/.test(CODE),
     '★★★ 時間一律走 shared/waittime.js —— 自己在頁面裡算會再長出'
     + '一個不會低估規則的版本');
  ok(/waittime\.js/.test(SRC),
     '★★★ 要真的把 waittime.js 載進來，不然 WaitTime 是 undefined');
  ok(/ocrAvg > 0/.test(CODE),
     '★★★ 問不到實測平均（0）時不可以估時間 —— 那時只講張數');
  ok(/setOcrAvg\(Number\(j\.avg_seconds\)/.test(CODE),
     '★★ 平均秒數要從後端拿，不是頁面自己訂');
  /* ⚠️ 寫死的數字是更早那次要消滅的東西 —— 兩學期都不可以再出現。 */
  ok(!/\*\s*6\s*\/\s*60/.test(CODE),
     '★★ 不可以再用寫死的 6 秒估時間');
  ok(!/const fmtWait|function fmtWait/.test(CODE),
     '★★★ 頁面裡不可以自己再定義一份 fmtWait（同一件事做兩次的老坑）');
  ok(/queue\.map\(/.test(CODE),
     '★★ 排隊清單仍然是主角（清單是事實，時間只是輔助）');
  ok(/不會不見/.test(SRC),
     '★★ 等的時候要講「圖已經在雲端，不會不見」—— '
     + '否則學生會重開視窗再傳一次，那正是故障自我放大的起點');
  /* ⚠️⚠️ 自己那一列的位置，和「還要等多久」必須用**同一個**算法。
     分開寫兩份的話症狀是「清單上標成你，估的時間卻是別人的」。 */
  ok(/const rowKey =/.test(CODE) && /rowKey\(row\) === myUpName/.test(CODE),
     '★★★ 標記「← 你」和估算位置要共用 rowKey');
  ok(/myIdx >= 0 \? myIdx : queue\.length/.test(CODE),
     '★★★ 清單裡還找不到自己時要當作排最後，不可以當作 0 ——'
