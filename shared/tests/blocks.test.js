...
(async function () {
  ...
  /* ⚠️ 2026-08-17：**兩關**沒有拼圖了。
       第 5 關 6-1-1：課本用圖解，是排序的觀念導入
       第 10 關 6-3-3：它比的是第 8、9 關寫過的程式，自己不該再寫一支
                       —— 原本那支「兩個停止條件」的完整版已搬到第 8 關
     ★ 兩關都改用「實作體驗」收尾（見 GRADING.GATE.NO_UPLOAD）。 */
  is(puzzles.slice().sort(),
     ['4-2-1', '4-2-2', '4-2-3', '4-3-1', '6-2-1', '6-2-2', '6-3-1', '6-3-2'],
     '★ 十關裡有八關有拼圖');
  is(ids.filter(id => !L[id].goal).sort(), ['6-1-1', '6-3-3'],
     '★★ 沒有拼圖的是第 5、10 關 —— 那兩關的主角是體驗，不是積木');
  puzzles.forEach(id => {
