...
/* ⚠️ 2026-08-26 實測迴歸：每次 OCR 呼叫 ≈ 9.3 秒固定開銷 ＋ 26.1 秒/MP。
   ★ 「呼叫次數本身就要錢」這件事一定要留在判讀裡 ——
     一張截圖固定跑兩次，光固定開銷就 18.6 秒，比任何倍率調整都大。
     少了這句，下次有人又會從「調倍率」開始猜。 */
ok(/呼叫次數/.test(NBCODE) && /9\.3/.test(NBCODE),
   '★★ 判讀要講出「每次呼叫的固定成本」');
ok(/挑戰失敗/.test(NBCODE),
