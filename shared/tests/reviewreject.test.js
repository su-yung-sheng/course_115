...
(async () => {
  ...
  section('★★★ 學生看得到（11501 流程圖／程式設計地圖）');
  {
    ...
    /* ⛔ 老師 2026-09-23 問：「學生重新繳交，訊息會不見嗎？」—— 不會。
       退件只有老師再給分（或撤回退件）才會消失，而學生端讀不到 Classroom，
       不可能知道他交了沒。⇒ 那就**講明白**，不然他會以為沒交成功而一直重傳。 */
    ok(/這段話要等老師再看過、給分之後才會消失，不用重複上傳/.test(F),
       '★★★ 要講明「重交之後這段話不會馬上消失」—— 不講的話學生會重複上傳，以為沒交成功');
  }
  section('★★★ 學生看得到（11502 程式設計地圖）');
  {
    ...
    ok(/這段話要等老師再看過、給分之後才會消失，不用重複上傳/.test(S),
       '★★★ 同上：重交之後訊息不會馬上消失，要先講');
  }
