...
/* ★ 空清單必須是「刻意關閉」，不是「不小心刪掉」
   2026-08-19 測試帳號 1410905 關閉之後，這份清單就是空的。
   問題是：空清單和「被誰誤刪了一行」長得一模一樣，而兩者的後果差很多 ——
   前者是「現在不該有人繞過學期鎖」，後者是「你以為還測得了下學期，其實不行」。
   ⇒ 要求原始碼裡留下 CLOSED 註記（關閉日期＋怎麼重開），
     讓「空的」這件事自己說得出理由。 */
if ((a || []).length === 0) {
  ok(/★ CLOSED \d{4}-\d{2}-\d{2}/.test(js),
     '★ 清單是空的，semester.js 要有 CLOSED 註記說明是刻意關閉（不是漏刪）');
  ok(/CLOSED[\s\S]{0,1200}?重新開一個測試帳號/.test(js),
     '   而且要寫清楚怎麼重開 —— 不然下次要測下學期會卡在「為什麼進不去」');
  ok(/★ CLOSED \d{4}-\d{2}-\d{2}/.test(rules),
     '★ 規則那邊也要有 CLOSED 註記 —— 它是唯一寫著「改完要按發布」的地方');
} else {
  ok(!/★ CLOSED/.test(js),
     '★ 清單有人了，就不可以還留著 CLOSED 註記（會讓人以為現在沒開）');
}

/* ⚠️ 函式本體不可以拆掉。
   清單空了很容易順手把 isTestAccount() 一起刪掉 ——
   但規則裡有四行 allow 還在呼叫它，Firestore 規則遇到未定義的函式
   會在「發布」那一步**整份編譯失敗**，於是整份規則一次都沒上線。
   這個專案 2026-08-06～08-11 就是這樣過了五天（症狀只有 permission-denied）。 */
ok(/function isTestAccount\(sid\)/.test(rules),
   '★ isTestAccount() 的函式本體還在（四行 allow 還在呼叫它，刪掉會整份規則發布失敗）');
