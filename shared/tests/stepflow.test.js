...
section('★★ 沒有回呼的步驟，一定要有「下一步」按鈕');
{
  ...
  /* ⚠️ 終點步驟不需要「出路」—— 它本來就是最後一步。
     ...
     ⚠️ 2026-08-24 修：`play` 原本也列在這裡，但它**早就不是終點了**——
        老師 2026-08-18 指出「實作體驗結束後不是應該進入 🏁 期末檢核？」，
        那次之後 play 會 onPass → advance，按鈕上的字也由關卡頁傳進去。
        測試沒跟著改，於是照舊要求它有「回地圖的連結」——
        ★ 這種紅燈最危險：它逼的是**把已經修好的東西改回去**。
        ⇒ play 現在走一般規則（要有回呼或下一步按鈕），而它兩個都有。 */
  const TERMINAL = ['test'];
  marks.forEach((m, i) => {
    const from = m.index;
