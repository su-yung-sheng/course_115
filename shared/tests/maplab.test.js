...
section('★★ 接回數學課：① 是正比，② 不是反比（老師 2026-08-24）');
{
  /* 老師：「暖身活動就是國中數學的正比與反比，連結數學課學過的正比反比概念」
     ⚠️⚠️ 但只有 ① 是正比。**② 不是反比。**
        我們的②（hi=50、out=100）：距離 10 → 80、距離 20 → 60
        真的反比 y=800/x：      距離 10 → 80、距離 20 → 40
        ★★ 「遞減＝反比」正是學生最常見的誤解 ——
           教材寫成反比會**強化**那個錯誤，而不是修正它。 */
  ok(M.mapv(10, 50, 0, 0, 100) === 80 && M.mapv(20, 50, 0, 0, 100) === 60,
     '★★ 距離兩倍時值是 60，不是 40 —— 所以它不是反比');
  ok(M.mapv(20, 50, 0, 0, 100) !== M.mapv(10, 50, 0, 0, 100) / 2,
     '   （反比的定義是「距離兩倍，值變一半」，這裡不成立）');
  /* 每多 1 公分少的量固定 —— 這才是它真正的性質。 */
  const step = [10, 11, 12, 13].map(d => M.mapv(d, 50, 0, 0, 100));
  ok(step[0] - step[1] === step[1] - step[2] && step[1] - step[2] === step[2] - step[3],
     '★ 每多 1 公分，少的量固定（' + (step[0] - step[1]) + '）—— 那是直線的性質');

  const n1 = M.mathNote(1, { hi: 50, out: 100 });
  const n2 = M.mathNote(2, { hi: 50, out: 100 });
  ok(/正比/.test(n1) && /兩倍/.test(n1), '★ ① 過關後點出「這就是數學課的正比」');
  ok(/不是.*反比/.test(n2), '★★ ② 過關後**明講它不是反比**');
  ok(/直線/.test(n2) && /固定少/.test(n2), '   並用「往下走的直線／固定少同樣多」描述它');

  /* ⚠️ 老師 2026-08-24：「還沒教到遞減，那只說明就好」
     ⇒ 不可以用還沒教到的名詞。 */
  const src = read('shared/maplab.js').replace(/\/\*[\s\S]*?\*\//g, '');
  ['遞減', '一次函數', '斜率', '雙曲線'].forEach(t => {
    ok(!src.includes(t), '★★ 學生看得到的文字不出現「' + t + '」（還沒教到）');
  });
  /* 而「反比」只能出現在「不是反比」那種句子裡。 */
  const revMentions = src.split('反比').length - 1;
  ok(revMentions > 0 && /不是\*\*反比|不是.{0,6}反比/.test(src),
     '★ 提到反比時一定是在說「它不是反比」');

  /* 除不盡那一組不可以硬報一個小數 —— 講「固定少同樣多」就好。 */
  const n2b = M.mathNote(2, { hi: 30, out: 100 });
  ok(/固定少同樣多/.test(n2b) && !/3\.33/.test(n2b),
     '★ 除不盡的組合不報小數，改說「固定少同樣多」');
}
