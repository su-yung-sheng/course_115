...
section('★★★ 要講明這一頁只有 9/10 之後的資料');
{
  /* ⛔⛔ 2026-09-18：我自己就是在這裡判斷錯的。
     看了這一頁的 2-1-1A（9/10 之後 15 筆、14 筆是 100），下了
     「這一關的評分沒有鑑別度」的結論，還建議老師把它改成範本比對。
     老師反問「但是 2-1-1A 曾經有一份打錯字變成 95 分」，
     去翻**成績紀錄**才發現那一關實際有 37 筆，
     分布 0／33／40／55／70／85／95／100 都有 —— 鑑別度一直都在。
     ★ 這一頁少的不是資料，是**一句話**。
       「最近這批學生做得好」和「評分看不出差別」，
       在畫面上長得一模一樣，但處置完全相反。 */
  ok(/function sinceNote/.test(js) && /sinceNote\(rows\)/.test(js),
     '★★★ 有這一段，而且真的畫出來');
  ok(/STATS_SINCE/.test(js) && /2026-09-10/.test(js),
     '★★ 要講明這份觀測資料是什麼時候開始收的');
  ok(/最近這批學生做得好/.test(js),
     '★★★ 要直接點出那個誤讀 —— 只說「資料從某天開始」，看的人不會自己想到後果');
  ok(/成績紀錄/.test(js),
     '★★ 而且要指路：完整的成績在教師端的成績紀錄裡');
  /* ⚠️ 要排在最上面。擺在最下面的警語等於沒有 —— 結論早就下完了。 */
  ok(js.indexOf('sinceNote(rows)') < js.indexOf('tiles(rows)'),
     '★★ 這一段要排在統計方塊**之前**（擺在最下面等於沒有）');
}
