...
const KNOWN = {
  ...
  /* ⚠️ 2026-08-11：這裡曾經有 11501/music.html，理由寫「沒人連」——
     ...
     ★ 教訓：要往 KNOWN 加東西之前，先確認「真的沒有人連」，
       不要因為工具說沒有就相信 —— 工具也會錯。 */
  /* ⚠️ 臨時工具（老師 2026-09-12）：上課現場開放卡關的學生先往下做。
     做的事和教師端主控台的 unlockUnit() 完全一樣，只是不用開 F12。
     ★ **刻意**沒有任何頁面連到它 —— 這樣不需要的時候直接刪檔就結束，
       系統其他部分完全不受影響。
     ⇒ 刪掉 11501/unlock.html 的時候，**這一行也要一起刪**
       （下面那條「KNOWN 裡沒有已經不存在的檔案」會提醒你）。 */
  '11501/unlock.html': '臨時的關卡開放工具，老師存書籤直接開；刻意不掛在系統裡，不用時直接刪檔',
  ...
  /* ✅ 2026-08-12：logic／sort／search 三支已經收進關卡裡了
     （BLOCK_LEVELS 的 material 欄位 → level.html 的補充教材面板），
     所以那三筆 KNOWN 已經刪掉 —— 它們現在是真的被連到的。
     ⚠️ 這一段留著是為了記住那條規則：
        KNOWN 是「我確認過，刻意留著」，不是待辦清單。
        一旦某支重新被連上，就要從這裡刪掉 ——
        不刪的話，哪天真的斷線了也不會有人發現。 */
};
