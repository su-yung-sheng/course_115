    ...
    { key: 'auto', t: '自動', by: '超音波距離感測器',
      ...
      /* ⚠️⚠️ 老師 2026-08-25：「自動模式 & 手動模式 對於 成果卡無關連性?」
         ...
         ⇒ 現在它決定**成果發表第二句的範例**，而且會檢查你寫的條件
           和你選的模式對不對得起來（只提醒、不擋）。 */
      /* ⚠️⚠️ 這一份範例要**和 SHOW_Q[1].slots 一格一格對齊**。
         老師 2026-08-25 回報：「（燈條）會 …有兩種? （馬達）會 …關閉?」
         ★ 病根：第二句拆成五格的時候，我改了 SHOW_Q 的範例，
           **卻忘了這裡還有一份**（上一輪為了「模式決定範例」加的）。
           於是（燈條）那格顯示「燈條亮起來、風扇開始轉」（兩種擠在一起），
           （馬達）那格顯示「兩個都關掉」—— 整個錯位。
         ⚠️ 這種錯不會報錯，也不影響判定，只有**照著範例填的人**會被誤導。
         ⇒ 下面那條測試釘死「兩份的格數必須一樣」。 */
      ph: ['例：距離小於 30 公分',
           '例：亮起暖黃色', '例：慢慢開始轉',
           '例：熄掉', '例：停下來'],
