...
window.CONFIG = {
  ...
  /* ⚠️⚠️ 備課用：把「依序開放」整個關掉，十關全部打開。 ⚠️⚠️
     ---------------------------------------------------------------
     ★ 為什麼開
       要再確認一次十關的內容，卻得先把前面每一關都排對流程圖、
       上傳作品讓 Colab 批改到 2⭐ —— 那不是謹慎，是讓人乾脆不看。

     ⚠️ 這是**這學期就要上的**模組，比 11502 危險得多。
       所以一定要有到期日：過了 OPEN_ALL_UNTIL，
       GRADING.openAll() 就回 false，依序開放自動回來。
       「記得開學前關掉」不是一個機制，是一個願望 ——
       忘記的成本要從「整學期沒有鎖」降到「某一天開始要照順序走」。

     ⇒ 開著的時候，闖關地圖會掛一條**橘色橫幅**（含到期日）。
       那不是裝飾，是為了讓「忘了關」不可能安靜發生。
     ⇒ 過期之後 shared/tests/openall.test.js 會變紅，提醒你把設定清掉。
     --------------------------------------------------------------- */
  OPEN_ALL_UNITS: true,
  OPEN_ALL_UNTIL: '2026-08-29',      // ← 開學前。請改成你自己的日期
